...
function release(type, files, parent, willForcePush, callback) {
  ...
  if (!packager) {
    ...
    choices.push({name:`Create the tag ${newVersion} but do not push to remote(only local)`, value: newVersion, _value:'dont-push'});
    ...
    inquirer.prompt([{
      ...
      if (version !== 'exit') {
        ...
        callback(true);
      } else if(!willForcePush || answers._value === 'dont-push'){
        git(version, tag, filenames , false);
      }else{
        callback(false);
      }
    });
